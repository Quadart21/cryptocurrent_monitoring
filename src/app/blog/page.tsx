...
export const metadata: Metadata = {
  title: "Блог",
  description:
    "Статьи о выборе обменников, безопасности и криптовалютных переводах.",
};
...
export default async function BlogIndexPage() {
  ...
  return (
    <div className="space-y-8">
      <Breadcrumbs
        items={[{ href: "/", label: "Главная" }, { label: "Блог" }]}
      />
      <div>
        <h1 className="font-display text-3xl font-semibold text-ink">Блог</h1>
        <p className="mt-2 max-w-2xl text-ink-muted">
          Полезные материалы: как не попасть на скам, чем отличаются сети
          стейблкоинов и как читать таблицу курсов.
        </p>
      </div>

      {posts.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-line px-4 py-10 text-center text-sm text-ink-muted">
          Скоро здесь появятся статьи. Загляните позже.
        </p>
      ) : (
